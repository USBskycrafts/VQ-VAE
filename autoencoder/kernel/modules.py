...
class VectorQuantizer(nn.Module):
    def __init__(self, n_e=8192, e_dim=256, beta=0.25):
        super().__init__()
        self.n_e = n_e
        self.e_dim = e_dim
        self.beta = beta
        self.embedding = nn.Embedding(n_e, e_dim)
        self.embedding.weight.data.uniform_(-1.0 / n_e, 1.0 / n_e)  # 使用均匀分布初始化
    ...
    def forward(self, z):
        ...
        # 使用与第二份代码相同的 perplexity 计算方式
        avg_probs = torch.mean(min_encodings, dim=0)
        perplexity = torch.exp(-torch.sum(avg_probs *
                               torch.log(avg_probs + 1e-10)))

        z_q = torch.matmul(min_encodings, self.embedding.weight).view(z.shape)
        q_loss = F.mse_loss(z_q.detach(), z) + self.beta * \
            F.mse_loss(z_q, z.detach())
        z_q = z + (z_q - z).detach()
        return q_loss, z_q.permute(0, 3, 1, 2).contiguous(), perplexity, min_encodings, min_encoding_indices
